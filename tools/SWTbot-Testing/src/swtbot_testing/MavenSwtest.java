...
/* check the next page (Choose from available portlet frameworks depending on which technology is most appropriate for this project.)*/
...
/*After created the project .check the java and xml files*/ 
        ...
        bot.tree().getTreeItem( TEXT_PROJECT_NAME ).getNode( "src" ).getNode( "main" ).getNode(
            "webapp" ).expand();
        bot.tree().getTreeItem( TEXT_PROJECT_NAME ).getNode( "src" ).getNode( "main" ).getNode(
