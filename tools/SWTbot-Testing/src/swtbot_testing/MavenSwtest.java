...
public class MavenSwtest{
	...
	public static void beforeClass()
	{
		...
		bot.toolbarDropDownButtonWithTooltip("Create a new Liferay Plugin Project").menuItem("New Liferay Plugin Project").click();
	}
...
/*如果Build type是 Ant (liferay-plugins-sdk)	*/	
		...
		if((bot.comboBoxWithLabel("Build type:").getText()).equals( "Ant (liferay-plugins-sdk)" )){
	        ...
	        String expected_plugintype[]={"Portlet","Service Builder Portlet","Hook","Layout Template","Theme","Ext","Web"};
...
/*校验初始值*/       
            ...
            bot.textWithLabel("Liferay Tomcat directory").setText("D:\\6.2\\liferay-portal-tomcat-6.2-ce-ga6-20160112152609836\\liferay-portal-6.2-ce-ga6");            
...
//finish之后check生成的java文件
            ...
            bot.tree().getTreeItem("CreateMavenProjectTesting").getNode("src").getNode("main").getNode("webapp").expand();
            bot.tree().getTreeItem("CreateMavenProjectTesting").getNode("src").getNode("main").getNode("webapp").getNode("view.jsp").select();
            assertTrue(bot.tree().getTreeItem("CreateMavenProjectTesting").getNode("src").getNode("main").getNode("webapp").getNode("view.jsp").isVisible());
		}
		else{
...
//((bot.comboBoxWithLabel("Build type:").getText()).equals( "Maven (liferay-maven-plugin)" ))
